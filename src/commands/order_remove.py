...
async def order_remove(interaction: Interaction, item_tag: str, price: float | None = None):
    ...
    async for session in get_session():
        ...
        deleted_buy = await delete_buy_orders(session, user_id, server_id, item_tag, price)

        deleted_sell = await delete_sell_orders(session, user_id, server_id, item_tag, price)
