...
async def drink(interaction: Interaction, direct_execution = True):
    ...
    async for session in get_session():
        ...
        if await check_has_water(session, interaction, player): return

        await remove_player_item(session, user_id, server_id, "Water", 1)
...
async def check_has_water(session, interaction, player):
    has_water = await has_player_item(session, player.id, player.server_id, "Water")
    if not has_water:
        embed = Embed(
            title="Error!",
